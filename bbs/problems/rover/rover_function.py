...
class RoverProblem():
    def __init__ (self, num_points=2, k=3, s=None, extra_domain=0.1, img_map=None):
        print('debug RoverProblem args:', dict(num_points=num_points, k=k, s=s, extra_domain=extra_domain, img_map=img_map))
        ...
        self.bounds = []
        for p in range(len(self.lb)):
            self.bounds.append((self.lb[p],self.ub[p]))

    def __call__(self, x):
        return -self.f(x)

def main(img_filename='hill_map.png'):
